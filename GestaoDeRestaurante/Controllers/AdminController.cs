    ...
    public class AdminController : ControllerBase
    {
            ...
            [FromQuery] DateTime? dataFim)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(status) && Enum.TryParse<StatusPedido>(status, out var statusPedido))
                    query = query.Where(p => p.Status == statusPedido);

                if (!string.IsNullOrEmpty(periodo) && Enum.TryParse<Periodo>(periodo, out var periodoPedido))
        ...
        /// <summary>
        /// Altera o status de um pedido com validação de transição
        /// </summary>
        ...
        public async Task<IActionResult> AlterarStatusPedido(int id, [FromBody] AlterarStatusPedidoDTO dto)
        {
            try
            {
                ...
                if (!Enum.TryParse<StatusPedido>(dto.NovoStatus, out var novoStatus))
                    return BadRequest(new { mensagem = "Status inválido. Use: Recebido, EmPreparo, Pronto, Entregue ou Cancelado." });

                if (!TransicaoStatusPedidoValida(pedido.Status, novoStatus))
                    return BadRequest(new { mensagem = $"Transição inválida: não é possível mudar de '{pedido.Status}' para '{novoStatus}'." });
            ...
            [FromQuery] DateTime? dataFim)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(status) && Enum.TryParse<StatusReserva>(status, out var statusReserva))
        ...
        /// <summary>
        /// Altera o status de uma reserva com validação de transição
        /// </summary>
        ...
        public async Task<IActionResult> AlterarStatusReserva(int id, [FromBody] AlterarStatusReservaDTO dto)
        {
            try
            {
                ...
                if (!Enum.TryParse<StatusReserva>(dto.NovoStatus, out var novoStatus))
                    return BadRequest(new { mensagem = "Status inválido. Use: Ativa, Confirmada, Cancelada ou Finalizada." });

                if (!TransicaoStatusReservaValida(reserva.StatusReserva, novoStatus))
                    return BadRequest(new { mensagem = $"Transição inválida: não é possível mudar de '{reserva.StatusReserva}' para '{novoStatus}'." });
        ...
        private static bool TransicaoStatusPedidoValida(StatusPedido atual, StatusPedido novo)
        {
        ...
        private static bool TransicaoStatusReservaValida(StatusReserva atual, StatusReserva novo)
        {
            return (atual, novo) switch
            {
                (StatusReserva.Ativa, StatusReserva.Confirmada) => true,
                (StatusReserva.Ativa, StatusReserva.Cancelada) => true,
                (StatusReserva.Confirmada, StatusReserva.Finalizada) => true,
